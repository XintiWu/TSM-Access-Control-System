...
curl -sf -H "X-API-Key: ${API_KEY}" -X POST "${ACCESS_URL}/access/swipe" \
...
echo "=== Step 1: Health ==="
if curl -sf "${REPORT_URL}/ui/" >/dev/null 2>&1; then
  echo '{"status": "ok", "url": "'"${REPORT_URL}/ui/"'"}' | jq .
else
  curl -sf "${REPORT_URL}/health" | jq .
fi

echo "=== Step 2: Personal (employee) ==="
curl -sf -H "X-API-Key: ${API_KEY}" -H "X-User-ID: ${EMPLOYEE_ID}" \
...
CODE=$(curl -s -o /dev/null -w "%{http_code}" -H "X-API-Key: ${API_KEY}" -H "X-User-ID: ${EMPLOYEE_ID}" \
